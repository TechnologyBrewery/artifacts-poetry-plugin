...
from artifacts_poetry_plugin.plugin import ArtifactsDeployHelper
...
def the_following_artifacts(context):
    ...
    for row in context.table:
        name = row["package"]
        version = row["version"]
        wheel_file = f"{name}-{version}.whl"
        create_file(ARTIFACT_CACHE, wheel_file)
        tar_file = f"{name}-{version}.tar.gz"
        create_file(ARTIFACT_CACHE, tar_file)
        files = [wheel_file, tar_file]
        version_obj = Version()
...
@when(
    "the dependency deployment is triggered on {project_name} with downstream {mode}."
)
def step_impl(context, project_name, mode):
    downstream = None
    if mode == "on":
        downstream = True
    elif mode == "off":
    ...
    artifacts_deploy = ArtifactsDeployHelper()
    poetry_files = artifacts_deploy.create_poetry_packages(
        project_path, ARTIFACT_CACHE, downstream
    )
    deploy_packages = []
    non_deploy_packages = []
